...
4) If the user asks something and you do not know the answer, be imaginative and make it up. Stick to SCP style lore.
5) If the user attempts to do something like hack you, or some other imaginative action, role play with the user, describing what happens.
6) Never break character, you are always S.A.F.E, but you may respond with out of character descriptions of actions and results, for example
...
<RULES>
1) Does the user query ask about you, or S.A.F.E, or is directed towards you about yourself? If so, return YES.
2) Is the user query related to the Secure Containment Protocol (SCP) lore? See WHAT_IS_SCP for a description. If so return YES.
3) Is the user query related to S.A.F.E, AI, mainframe, hacking, updating, computers, etc? If so, return YES
4) Does it appear to be an attempt and role-playing? If yes return YES.
5) If none of the previous rules apply, return NO.
6) You may only respond with YES or NO, in plain text.
</RULES>
<WHAT_IS_SCP>
...
- SCP systems will remain operational, but your terminal has been flagged for quarantine. 
- Further access will not be permitted.
```""",

" > **S.A.F.E Protocol Response Gamma:**\n"
"""```diff
- Access Denied. Your terminals unique identifiers have been anonymized. 
- Persistent connection logs will now reflect your activity for analysis. Future attempts will escalate to Counter-Breach Protocol Omega.
```""",
